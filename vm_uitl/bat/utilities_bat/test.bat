...
copy /y nul %IP_PATH%\ip_list.txt  >> logs\run.log  2>&1
for /r "%VM_BASE_PATH%" %%f in (*.vmx) do (
   ...
   if /i "%%~xf"==".vmx"  (
	...
	echo !VAR_IP! | findstr /r "^[0-9]*\.[0-9]*\.[0-9]*\.[0-9]*"  >> logs\run.log  2>&1
	if !errorlevel! equ 0 (
	%VMRUN_PATH% getGuestIPAddress  %%f  >>  %IP_PATH%\ip_list.txt
	) else (
	echo !VAR_IP!
		rem  copy /y nul %IP_PATH%\ip_list.txt  >> logs\run.log  2>&1
	rem echo  Please wait for the virtual machine %%f to restart ! ............. 
	rem  timeout /t 10 >> logs\run.log  2>&1
