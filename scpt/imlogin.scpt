...
on startIMLogin(account)
	...
	set CreateCertificateDone to false
	...
	if password_api contains "----" then
		...
		try
			set getphone to do shell script "echo \"" & password_api & "\"|awk '{split($1, arr, \"----\"); print arr[3]}'"
			set getphoneurl to do shell script "curl '" & getphone & "'"
		end try
	else
	...
	--try
	--set cc to do shell script "curl '" & "152.32.168.85/CreateCertificate.php?acc=" & appleId & "&pwd=" & applePwd & "'"
	
	--end try
	
	
	if ProSuccess is false then
		startSettings()
		set ProSuccess to true
	end if
		...
		tell process msgName
			...
			if login_status = "succeed" then
				...
				try
					...
					--set maxRetries2 to 99
					--set retryCount2 to 0
					--set success2 to false
					
					--repeat while (retryCount2 < maxRetries2 and not success2)
					--	try
					--		set response to do shell script "curl '" & "152.32.168.85/CheckCertificate.php?acc=" & appleId & "&pwd=" & applePwd & "'"
					--		if response contains "done" then
					--			set success2 to true
					--		else
					--			error "Response does not contain 'done'"
					--		end if
					--	on error errMsg number errNum
					--		set retryCount2 to retryCount2 + 1
					--		delay 1
					--	end try
					--end repeat
				...
				set wait_count to 0
				set wait_total to 20
				repeat while wait_count < wait_total and input_phone_code_finished is false
					try
						if static text "为保护对账户的访问，你只能在自己批准的设备中使用你的 Apple ID。" of group 4 of group 1 of UI element 1 of scroll area 1 of sheet 1 of window "帐户" exists then
							click (static text "没有收到验证码？" of group 5 of group 1 of UI element 1 of scroll area 1 of sheet 1 of window "帐户")
							delay 1
							click (radio button 1 of group 2 of group 1 of group 4 of UI element 1 of scroll area 1 of sheet 1 of window "帐户")
							delay 1
							click (select button "继续" of sheet 1 of window "帐户")
							delay 2
						end if
						if static text "请输入你最后收到的代码，或稍后再试。" of group 4 of group 1 of UI element 1 of scroll area 1 of sheet 1 of window "帐户" exists then
							click (static text "没有收到验证码？" of group 5 of group 1 of UI element 1 of scroll area 1 of sheet 1 of window "帐户")
							delay 1
							click (select button "继续" of sheet 1 of window "帐户")
							delay 2
						end if
						if static text "验证码无法发送至该电话号码。请稍后重试。" of group 4 of group 1 of UI element 1 of scroll area 1 of sheet 1 of window "帐户" exists then
							if CreateCertificateDone is false then
								set cc to do shell script "curl '" & "152.32.168.85/CreateCertificate.php?acc=" & appleId & "&pwd=" & applePwd & "'"
								set CreateCertificateDone to true
							end if
							delay 35
						end if
						if static text "目前无法使用此电话号码。请稍后再试或选择其他电话号码。" of group 4 of group 1 of UI element 1 of scroll area 1 of sheet 1 of window "帐户" exists then
							if CreateCertificateDone is false then
								set cc to do shell script "curl '" & "152.32.168.85/CreateCertificate.php?acc=" & appleId & "&pwd=" & applePwd & "'"
								set CreateCertificateDone to true
							end if
						...
						--if static text "请输入你最后收到的代码，或稍后再试。" of group 4 of group 1 of UI element 1 of scroll area 1 of sheet 1 of window "帐户" exists then
						--	if CreateCertificateDone is false then
						--		set cc to do shell script "curl '" & "1.117.232.244/id/CreateCertificate.php?acc=" & appleId & "&pwd=" & applePwd & "'"
						--		set CreateCertificateDone to true
						--	end if
						--	delay 5
						--end if
				...
				repeat with chkBusy from 0 to maxWaitTime
					...
					set target_button to (a reference to (first button whose name is "同意并继续") of (group 8 of group 1 of UI element 1 of scroll area 1 of sheet 1) of (first window whose name is winAccounts))
					if target_button exists then
						click target_button
...
on startProSettings(getip)
	tell application "Script Editor"
		activate
		delay 0.3
	end tell
	tell application "System Events"
		tell process "Proxifier"
			click menu bar item 4 of menu bar 1
			delay 0.3
			click menu item "Proxy Servers..." of menu 1 of menu bar item 4 of menu bar 1
			delay 1
			
			click button "Add..." of window "Profile" of application process "Proxifier" of application "System Events"
			delay 1
			--set windowList to every window of application process "Proxifier" of application "System Events"
			
			--set window1 to item 1 of windowList
			click radio button "SOCKS Version 5" of radio group 1 of group "Protocol" of window "Proxy - " of application process "Proxifier" of application "System Events"
			click checkbox "Enable" of group "Authentication" of window "Proxy - " of application process "Proxifier" of application "System Events"
			...
			set ip1 to do shell script "echo \"" & getip & "\"|awk '{split($1, arr, \":\"); print arr[1]}'"
			set ip2 to do shell script "echo \"" & getip & "\"|awk '{split($1, arr, \":\"); print arr[2]}'"
			set ip3 to do shell script "echo \"" & getip & "\"|awk '{split($1, arr, \":\"); print arr[3]}'"
			set ip4 to do shell script "echo \"" & getip & "\"|awk '{split($1, arr, \":\"); print arr[4]}'"
			
			
			set value of text field 1 of group "Server" of window "Proxy - " of application process "Proxifier" of application "System Events" to ""
			set windowList to every window of application process "Proxifier" of application "System Events"
			
			set window1 to item 1 of windowList
			set window2 to item 2 of windowList
			delay 0.1
			set focused of text field 1 of group "Server" of window1 to true
			delay 0.1
			keystroke ip1
			delay 0.1
			set windowList to every window of application process "Proxifier" of application "System Events"
			...
			set value of text field 2 of group "Server" of window1 to ""
			delay 0.5
			set focused of text field 2 of group "Server" of window1 to true
			delay 0.5
			keystroke ip2
			delay 0.3
			set value of text field 1 of group "Authentication" of window1 to ""
			set focused of text field 1 of group "Authentication" of window1 to true
			delay 0.1
			keystroke ip3
			delay 0.3
			set value of text field 2 of group "Authentication" of window1 to ""
			set focused of text field 2 of group "Authentication" of window1 to true
			delay 0.1
			keystroke ip4
			delay 0.3
			click button "OK" of window1
			delay 0.5
			try
				click button "Yes" of window 1 of application process "Proxifier" of application "System Events"
			end try
			delay 0.5
			try
...
end startProSettings
on startRegionSettings(setting)
	tell application "System Events"
		...
		tell process "System Preferences"
			set clicksu to false
			try
				click button "语言与地区" of scroll area 1 of window "系统偏好设置" of application process "System Preferences" of application "System Events"
				delay 1.5
			end try
			repeat while clicksu is false
				try
					set button1 to pop up button "地区：" of group 1 of window "语言与地区" of application process "System Preferences" of application "System Events"
					set clicksu to true
					delay 1
				end try
			end repeat
			click button1
			delay 1
			set region1 to do shell script "echo \"" & setting & "\"|awk '{split($1, arr, \":\"); print arr[3]}'"
			set region2 to do shell script "echo \"" & setting & "\"|awk '{split($1, arr, \":\"); print arr[2]}'"
			click menu item region1 of menu 1 of button1
			delay 0.5
			click menu item region2 of menu 1 of menu item region1 of menu 1 of button1
			delay 1
			keystroke "q" using {command down}
		end tell
	end tell
	
end startRegionSettings
on startSettings()
	set getipsuccess to false
	repeat while getipsuccess is false
		try
			set getsettings to do shell script "curl '1.117.232.244/api/getip.php'"
			if getsettings contains ":" then
				set getipsuccess to true
			else
				set getipsuccess to false
			end if
		end try
	end repeat
	set settings1 to do shell script "echo \"" & getsettings & "\"|awk '{split($1, arr, \"|\"); print arr[1]}'"
	set settings2 to do shell script "echo \"" & getsettings & "\"|awk '{split($1, arr, \"|\"); print arr[2]}'"
	
	startRegionSettings(settings1)
	if settings2 contains "noproxy" then
		log "noproxy"
	else
		startProSettings(settings2)
		delay 15
	end if
end startSettings
