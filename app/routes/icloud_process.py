...
# 进程管理路由
@icloud_process_bp.route('/api/process/list')
...
@icloud_process_bp.route('/api/process/add', methods=['POST'])
def add_process():
    data = request.json
    new_process = {
...
@icloud_process_bp.route('/api/process/start', methods=['POST'])
def start_process():
...
@icloud_process_bp.route('/api/process/stop', methods=['POST'])
def stop_process():
    process_id = request.json.get('process_id')
    for process in processes:
        if process['id'] == process_id:
